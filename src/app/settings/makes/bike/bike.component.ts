...
export class BikeComponent implements OnInit {
  ...
  get makef() { return this.makeForm.controls; }
  ...
  onSubmitData() {
    ...
    if (this.isEdit) {
      this.makeEditService(this.makeForm.value)
      return;
    }
    this.submitted = false;
    this.makeForm.value.type = 'BIKE';
    this.authService.addMakes(this.makeForm.value)
      .subscribe((res: any) => {
        if (res.success == true) {
  ...
  makeEditService(data) {
    this.authService.editMakes(data, this.makeId)
      .subscribe((res: any) => {
        if (res.success == true) {
          this.toastr.success('Success ', res.massage);
          this.makeForm.reset();
  ...
  changeStatus(value) {
    ...
    this.authService.editMakes(object, value.id)
      .subscribe((res: any) => {
        if (res.success == true) {
          this.toastr.success('Success ', res.massage);
          this.ngOnInit();
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire(
          ...
          this.authService.deleteMakes(value).subscribe((res: any) => {
            if (res.success == true) {
