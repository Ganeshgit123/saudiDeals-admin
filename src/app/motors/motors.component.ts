...
export class MotorsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.displayedColumns = ['index', 'title', 'status', 'userId', 'provinceName', 'cityName', 'mainMotorCategoryName', 'price',
      'date', 'details', 'action'];

    // Restore selected status filter if saved
    const savedFilter = localStorage.getItem('motorsStatusFilter');
    if (savedFilter) {
      this.selectedStatus = savedFilter;
    }

    this.authService.getMotorsList(1).subscribe(
      (res: any) => {
        ...
        const savedState = localStorage.getItem('motorsListState');
        let filter = '';
        let pageIndex = 0;
        let pageSize = 10;
        ...
        setTimeout(() => {
          ...
          localStorage.removeItem('motorsListState');
        });
      });
  ...
  goToDetails(id: number) {
    ...
    localStorage.setItem('motorsListState', JSON.stringify(state));

    this.router.navigate(['/view_motors', id]);
  }
  ...
  onChangeStatus(value, id, Reason) {
    ...
    if (status != 2) {
      ...
      this.authService.approveMotorPost(object, this.postId)
        .subscribe((res: any) => {
          if (res.success == true) {
  ...
  onSubmitData() {
    ...
    this.authService.rejectMotorPost(this.rejectForm.value, this.postId)
      .subscribe((res: any) => {
        if (res.success == true) {
          this.toastr.success('Success ', res.massage);
  ...
  onChangeStatusTypeFilter(value: string) {
    ...
    localStorage.setItem('motorsStatusFilter', value);

    if (value === '3') {
      this.authService.getExpiredMotors(1).subscribe((res: any) => {
        this.displayedColumns = ['index', 'title', 'userId', 'provinceName', 'cityName', 'mainMotorCategoryName', 'price', 'date'];
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire(
          ...
          this.authService.approveMotorPost(object, value.id)
            .subscribe((res: any) => {
              if (res.success == true) {
  ...
  ngOnDestroy(): void {
    ...
    // Only remove if NOT going to /view_motors/:id
    const isGoingToView = nextUrl.startsWith('/view_motors');

    if (!isGoingToView) {
      localStorage.removeItem('motorsStatusFilter');
    }
