...
@Component({
  selector: 'app-motor-subsriiption',
  templateUrl: './motor-subsriiption.component.html',
  styleUrls: ['./motor-subsriiption.component.scss'],
  standalone: false
})
export class MotorSubsriiptionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.authService.getSubcPackage('MOTOR').subscribe(
      (res: any) => {
        this.getMotorSubsc = res.data;
        this.dataSource = new MatTableDataSource(this.getMotorSubsc);
        this.dataSource.paginator = this.matPaginator;
        this.dataSource.sort = this.matSort;
      }
    );

    this.motorSubForm = this.fb.group({
      userType: [''],
  ...
  get motF() { return this.motorSubForm.controls; }
  ...
  onSubmitData() {
    ...
    if (this.motorSubForm.invalid) {
      return;
    }

    if (this.isEdit) {
      this.subscEditService(this.motorSubForm.value)
      return;
    }
    this.submitted = false;
    this.motorSubForm.value.type = "MOTOR"
    this.motorSubForm.value.userType = "DEALER"
    this.authService.addSubcPackage(this.motorSubForm.value)
      .subscribe((res: any) => {
        if (res.success == true) {
